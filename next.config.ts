...
// `unsafe-eval` is added in development only: Next's Fast Refresh runtime
// (@next/react-refresh-utils) calls eval() to apply hot updates, and that
// code doesn't exist at all in a production build — so this never weakens
// the CSP actually shipped to real visitors.
